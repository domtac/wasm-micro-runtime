...
#define CALCULATE_DEFAULT_MAX_SIZE(is_memory64, num_bytes_per_page) \
    ({ \
        uint32 _custom_page_multiplier = (DEFAULT_NUM_BYTES_PER_PAGE / (num_bytes_per_page)); \
        uint32 _default_max_size = (is_memory64) ? DEFAULT_MEM64_MAX_PAGES : DEFAULT_MAX_PAGES; \
...
static bool
check_memory_init_size(bool is_memory64, uint32 init_size, uint32 num_bytes_per_page, char *error_buf,
                       uint32 error_buf_size)
{
    uint32 default_max_size = CALCULATE_DEFAULT_MAX_SIZE(is_memory64, num_bytes_per_page);
...
static bool
check_memory_max_size(bool is_memory64, uint32 init_size, uint32 num_bytes_per_page, uint32 max_size,
                      char *error_buf, uint32 error_buf_size)
{
    uint32 default_max_size = CALCULATE_DEFAULT_MAX_SIZE(is_memory64, num_bytes_per_page);
                   ...
                   char *error_buf, uint32 error_buf_size)
{
    ...
    if (!check_memory_init_size(is_memory64, declare_init_page_count, num_bytes_per_page, error_buf,
                                error_buf_size)) {
        return false;
    }
    ...
    if (mem_flag & MAX_PAGE_COUNT_FLAG) {
        ...
        if (!check_memory_max_size(is_memory64, declare_init_page_count, num_bytes_per_page,
            ...
            char *error_buf, uint32 error_buf_size)
{
    ...
    if (!check_memory_init_size(is_memory64, memory->init_page_count, num_bytes_per_page, error_buf,
                                error_buf_size))
        return false;
    ...
    if (memory->flags & 1) {
        ...
        if (!check_memory_max_size(is_memory64, memory->init_page_count, num_bytes_per_page, 
                   ...
                   bool no_resolve, char *error_buf, uint32 error_buf_size)
{
    ...
    if (!module->possible_memory_grow) {
        ...
        if (module->import_memory_count) {
            WASMMemoryImport *memory_import =
                &module->import_memories[0].u.memory;
            uint32 custom_pages_multiplier = DEFAULT_NUM_BYTES_PER_PAGE / memory_import->mem_type.num_bytes_per_page;
